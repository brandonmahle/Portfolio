...
def create_table():
    with connection: 
        ...
        connection.execute(CREATE_USERS_TABLE) # this must be created second because of the foreign keys in watched
...
def get_movies(upcoming = False):
    with connection:
        cursor = connection.cursor() #could also do cursor = connection.execute(SELECT_ALL_MOVIES)
        if upcoming:
